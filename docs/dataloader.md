...
2. **Streaming JSONL units** – read one entry per line and branch based on `serialization_mode`:
   - `pairs`: validate canonical/embedded SMILES (minimum embedded length `min_emb_len=16`), build the unit string via `build_unit(...)`, and tokenize without special tokens.
   - `isomer_units`: tokenize `[SMILES]...[/SMILES]` once per isomer, tokenize conformers, and chunk so each emitted unit contains the SMILES plus one or more conformers.
3. **Packing sequences** – units are appended with a single delimiter token (the tokenizer’s pad token; `<|endoftext|>` when auto-built). When no more units fit, the remainder is padded with that same token. Labels are shifted left by one token (next-token prediction) and any suffix without a real successor—including the final position and all padding—is set to `ignore_index=-100`. There is no BOS/EOS—every sample starts with a fresh unit, and `seq_len` remains configurable (default 2048).
4. **Buffered shuffle + lookahead** – maintain a shuffle buffer (default 4096 entries) per worker and refill it deterministically per epoch/seed/rank.
   - `pairs`: uses a lookahead buffer (`lookahead_limit`, default 100) and selects the unit that best fills remaining space.
   - `isomer_units`: streams units in order; each unit is atomic and never split.
...
6. **Monitoring** – `_PreviewLogger` logs the first two decoded sequences per rank; `count_tokens.py` provides aggregated diagnostics and statistics.
...
- Supports `pairs` and `isomer_units`, and multiple tokenizers simultaneously for comparison.
...
python -m molgen3D.training.pretraining.dataprocessing.count_tokens \
  --serialization-mode pairs

python -m molgen3D.training.pretraining.dataprocessing.count_tokens \
  --serialization-mode isomer_units \
  --fast-estimate
```
...
- **Atomic units**: `pairs` never splits `[SMILES]…[/SMILES][CONFORMER]…[/CONFORMER]` units; `isomer_units` never splits a SMILES+conformer chunk. When a tokenized unit exceeds `seq_len - 1` tokens, truncate it in-place (logging once per rank) so the remaining prefix still participates in training. Set `truncate_overflow_units=False` to revert to the old skip-only behavior.
...
- **Buffered shuffle + limited lookahead**: use a moderate shuffle buffer (default 4096). In `pairs` mode, use a lookahead buffer (`lookahead_limit`, default 100) to pick the best-fitting unit; in `isomer_units`, stream units in order.
