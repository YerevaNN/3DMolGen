...
def test_conformer_coordinates(xyz_aspirin_molecule, cif_aspirin_molecule):
    ...
    mol_xyz = convert_xyz_to_mol(
        XYZ_PATH, canonicalize_smi=True, canonicalize_atoms=True
    )
    mol_cif = convert_cif_to_mol(
        CIF_PATH, canonicalize_smi=True, canonicalize_atoms=True
    )
    ...
    assert [a.GetAtomicNum() for a in mol_xyz.GetAtoms()] == [
        a.GetAtomicNum() for a in mol_cif.GetAtoms()
    ]
