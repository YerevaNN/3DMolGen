"""
AIMNet2-based molecular energy and forces calculation.

Adapted from LoQI repository (https://github.com/isayevlab/LoQI)
for 3DMolGen molecular conformation generation evaluation.
"""
...
from molgen3D.evaluation.aimnet2_utils import (
    check_topology,
    group_opt,
...
def aggregate_dict(dct: Dict, agg_idx: int) -> Dict:
    """
...
def compute_distance(pairs, agg_idx: int, compute_function) -> float:
    """
...
def prepare_for_aimnet(rdkit_molecules: List[Chem.Mol], device: str = "cpu") -> Dict[str, torch.Tensor]:
    """
...
def prepare_for_aimnet_chunked(
    rdkit_molecules: List[Chem.Mol],
    device: str = "cpu",
    chunked: bool = True,
    min_chunk_size: int = 1000
) -> Tuple[List[List[int]], List[Dict[str, torch.Tensor]]]:
    """
    Prepare RDKit molecules for AIMNet2.
...
def check_topology_wrapper(rdkit_mol: Chem.Mol) -> bool:
    """
...
class Forces(nn.Module):
    """Module to compute forces from energy gradients."""
...
class MoleculeAIMNet2Metrics:
    ...
    def __init__(self, model_path: str, batchsize: int = 32, opt_metrics: bool = False,
                 device: str = "cpu", opt_params: Optional[Dict] = None, chunked: bool = False):
    ...
    @torch.no_grad()
    def __call__(self, molecules: List[Chem.Mol], reference_molecules: Optional[List[Chem.Mol]] = None,
                 return_molecules: bool = False) -> Union[Dict[str, float], Tuple[Dict[str, float], List[Chem.Mol], Optional[List[Chem.Mol]], torch.Tensor]]:
        """
        Compute molecular metrics.

        Args:
            molecules (list): List of RDKit molecule objects.
            reference_molecules (list, optional): Reference molecules for relative energy calculation.
            return_molecules (bool): Whether to return optimized molecules.

        Returns:
            dict or tuple: Computed metrics, optionally with molecules and energies.
        """
        ...
        energy = torch.zeros(len(valid_molecules), device=self.device, dtype=torch.double)
        max_forces = torch.zeros(len(valid_molecules), device=self.device, dtype=torch.float)
        ...
        if self.opt_metrics:
            ...
            valid_molecules, opt_molecules, res_energy = self.compute_optimized_metrics(
    ...
    def calculate_energy_forces_batched(self, aimnet2_batch: Dict[str, torch.Tensor]) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        ...
        for start_idx in range(0, len(coords), self.batchsize):
            sub_batch = {k: v[start_idx: start_idx + self.batchsize] for k, v in aimnet2_batch.items()}
    ...
    def compute_optimized_metrics(self, aimnet2_chunk: List[Dict], aimnet2_idxs: List[List[int]],
                                  valid_molecules: List[Chem.Mol], energy: torch.Tensor,
                                  metrics: Dict, ref_energy: Optional[torch.Tensor] = None) -> Tuple[List[Chem.Mol], List[Chem.Mol], torch.Tensor]:
        """
...
def calculate_energies(molecules: List[Chem.Mol], aimnet2_model_path: str,
                      device: str = "cpu", batch_size: int = 32) -> Dict[str, Union[float, List[float]]]:
    """
    Simple interface to calculate energies for molecules using AIMNet2.

    Args:
        molecules: List of RDKit molecules
        aimnet2_model_path: Path to AIMNet2 model
