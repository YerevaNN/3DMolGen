"""
Constrained Conformer Generation - v1 Baseline

Version 1: Structural constraints only, no coordinate constraints.

This baseline tests the model's natural ability to generate coordinate blocks.
- SMILES structure (atoms, bonds, rings) is 100% forced to match input
- Coordinate blocks are completely unconstrained
- No blocklists, no allowlists inside <x,y,z>
"""
...
class ConformerConstraintLogitsProcessorV1(LogitsProcessor):
    """
    V1 Baseline: Structural constraints only.

    Forces exact SMILES structure (atoms, bonds, rings, branches) while leaving
    coordinate blocks completely unconstrained to test model's natural capability.
    """

    VERSION = "v1_structural_only"
    CONFIG = {
        "approach": "structural_only",
        "coord_constraints": None,
        "description": "Forces SMILES structure, zero coordinate constraints"
    }
        ...
        for b in range(batch_size):
            ...
            if seg.kind == "fixed":
                ...
                if state.coord_prefix_offset < len(prefix):
                    ...
                    # V1: DO NOTHING - let model generate freely
                    # No masking, no constraints
                    pass
