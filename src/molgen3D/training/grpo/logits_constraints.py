...
def attach_conformer_controls(model, tokenizer, config) -> None:
    """Attach conformer control processors and stopping criteria to a model.
    
    This modifies the model's _get_logits_processor and _get_stopping_criteria
    methods to automatically include conformer control logic during generation.
    
    Args:
        model: The language model to modify
        tokenizer: The tokenizer
        config: Config object with grpo.target_conformers, model.conf_tags, model.mol_tags
    """
    ...
    banned_ids.update(smiles_end_ids)  # Also ban closing SMILES tag!
