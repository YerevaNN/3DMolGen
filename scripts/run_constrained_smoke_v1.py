#!/usr/bin/env python
"""
Run constrained conformer smoke test using v1 baseline processor.

v1: Structural constraints only - no coordinate constraints.
Tests the model's natural ability to generate <x,y,z> blocks.
"""
...
from molgen3D.evaluation.constrained_logits_v1 import (
    ConformerConstraintLogitsProcessorV1,
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run v1 constrained conformer smoke test")
...
def _build_metadata(args: argparse.Namespace) -> dict:
    ...
    return {
        "version": ConformerConstraintLogitsProcessorV1.VERSION,
        "timestamp": datetime.now().isoformat(),
        "model_checkpoint": str(model_path),
        "tokenizer": args.tokenizer_name,
        "logit_processor_config": ConformerConstraintLogitsProcessorV1.CONFIG.copy(),
        "generation_config": {
...
def _generate_constrained(
    ...
    """Generate conformers with v1 structural constraints."""
    prompts = [f"[SMILES]{s}[/SMILES]" for s in smiles_list]

    # Use tokenizer's actual EOS token, not [/CONFORMER]
    # The processor will force [/CONFORMER] via templates, THEN force EOS
    eos_token_id = tokenizer.eos_token_id
    if eos_token_id is None:
        # Fallback: use pad token as EOS
    ...
    with torch.inference_mode():
        for start in range(0, len(prompts), batch_size):
            ...
            # Build v1 processor with EOS token
            ...
            processor = ConformerConstraintLogitsProcessorV1(
                templates,
                prompt_lengths,
                tokenizer=tokenizer,
...
def main() -> int:
    ...
    logger.info("Starting v1 constrained generation (structural only).")
