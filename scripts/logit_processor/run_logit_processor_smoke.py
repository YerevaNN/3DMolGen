...
def build_templates_parallel(
    ...
    Args:
        ...
        processor_type: "qwen" (only option, kept for API compatibility)
    ...
    build_with_tokenizer = partial(
        build_precomputed_template, tokenizer=tokenizer, use_tokenizer_cache=use_tokenizer_cache
    )
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--model-alias", default="m600_qwen_pre")
    parser.add_argument("--model-step", default="2e")
    parser.add_argument("--model-path", type=Path)
    parser.add_argument("--tokenizer-name", default="qwen3_0.6b_custom")
    ...
    parser.add_argument("--processor-type",
                        choices=["qwen"],
                        default="qwen",
                        help="Logit processor type: 'qwen' (v4.3 strict allowlist)")
    # Optimization flags
...
def _get_processor_class_and_template_fn(processor_type: str):
    ...
    if processor_type == "qwen":
        return QwenAllowlistLogitsProcessor, build_precomputed_template
    else:
        raise ValueError(f"Unknown processor type: {processor_type}. Only 'qwen' is supported.")
...
def run_smoke_test(config: dict) -> int:
    ...
    if args.json_report:
        ...
        if use_lp:
            processor_version = QwenAllowlistLogitsProcessor.VERSION
        else:
