...
def obabel_convert(
    ...
    canonicalize_smi: bool = True,
    canonicalize_atoms: bool = False,
):
    ...
    with tempfile.NamedTemporaryFile() as tmp:
        ...
        if canonicalize_atoms:
            canonical_order = [
                int(i)
                for i in mol.GetProp("_smilesAtomOutputOrder")[1:-1].split(",")
                if i != ""
            ]
...
def convert_cif_to_mol(
    file_path: str, canonicalize_smi: bool = True, canonicalize_atoms: bool = False
):
    obConversion = openbabel.OBConversion()
